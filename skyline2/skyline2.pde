// Think about your future self, self!
// http://threepanelsoul.com/2013/12/16/on-perl/


int blocksBuilt = 0; //why the fuck are you the only variable that functions can see?
float cityGround = 0;
int moonX = 0;
int moonY = 0;
float moonRise = 0;
...
void draw() {
  int s = second();
  int m = minute();
  float minutesInPi = m*.01;
  float tester = sin(((minutesInPi*PI)));
  moonYDisplace = int(tester*40);
  //  moonCycle = int(second()*width);
  moonCycle = abs(-int(30-second()));

  print("MoonX:   "+moonX+'\n');
  print("MoonY:   "+moonY+'\n');
  print("secondsInPi:   "+minutesInPi+'\n');
  print("tester:   "+tester+'\n');
  print("moonYDisplace:   "+moonYDisplace+'\n');
...
void drawMoon() {
  ...
  moonX = minute()*moonMoveDistance;
  moonY = 100-moonYDisplace;
  ...
  fill(#FFFDED);
  // previous fading fill value   fill(#FFFDED, moonFill);
  ellipse(moonX, moonY, moonDiameter, moonDiameter);
